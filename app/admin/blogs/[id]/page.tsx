...
import { adminUpdateBlog } from "../actions";
...
export default async function AdminEditBlogPage({ params }: Props) {
  ...
  if (!blog || blog.type !== "blog") notFound();
  ...
  return (
    <>
      <div className="admin-topbar mb-8">
        ...
        <h1 className="text-3xl font-black text-[#10233f]">Edit Blog Column</h1>
        <p className="text-slate-500 text-sm mt-1">Modify details for blog &quot;{blog.title}&quot;.</p>
      </div>

      <form action={adminUpdateBlog} className="bg-white rounded-lg border border-slate-200 shadow-sm p-8 max-w-[900px]">
        <input type="hidden" name="id" value={blog.id} />
        <div className="grid gap-6">
          {/* Title */}
          <div>
            <label className="block text-xs font-black uppercase text-[#10233f] tracking-wide mb-2">
              Blog Title <span className="text-[#b42318] font-bold">*</span>
            </label>
            <input
              type="text"
              name="title"
              defaultValue={blog.title}
              placeholder="e.g. Modern Physics and PBL inside Eureka"
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g. modern-physics-and-pbl"
              className="w-full bg-[#f8fafa] border border-slate-200 rounded px-4 py-2.5 text-slate-800 outline-none focus:ring-2 focus:ring-[#3eaea6] focus:bg-white transition"
            />
          </div>

          {/* Homepage order & Pinned */}
          <div className="grid grid-cols-2 gap-4 max-sm:grid-cols-1">
            <div>
              <label className="block text-xs font-black uppercase text-[#10233f] tracking-wide mb-2">
                Homepage order <span className="text-slate-400 font-normal">(Optional numeric display priority)</span>
              </label>
              <input
                type="number"
                name="homepage_order"
                defaultValue={blog.homepage_order ?? ""}
                placeholder="e.g. 1 (First), 2 (Second), leave empty for default chronological"
                className="w-full bg-[#f8fafa] border border-slate-200 rounded px-4 py-2.5 text-slate-800 outline-none focus:ring-2 focus:ring-[#3eaea6] focus:bg-white transition"
              />
            </div>
            <div>
              ...
              <select
                name="pinned"
                className="w-full bg-[#f8fafa] border border-slate-200 rounded px-4 py-2.5 text-slate-800 outline-none focus:ring-2 focus:ring-[#3eaea6] focus:bg-white transition"
                defaultValue={blog.pinned ? "true" : "false"}
              >
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              placeholder="Enter a brief, engaging summary of the article for blog listings..."
          ...
          <div>
            <label className="block text-xs font-black uppercase text-[#10233f] tracking-wide mb-2">
              Article Content (Rich Text / Markdown Toolbar supported)
            </label>
            <RichTextEditor
              name="body"
              defaultValue={blog.body}
              placeholder="Write column content here. You can select text and click B to bold, I to italic, etc."
              rows={12}
            />
          </div>
